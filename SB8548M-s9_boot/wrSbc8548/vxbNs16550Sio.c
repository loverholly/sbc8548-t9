...
#define NS16550_MAX_RATE 921600
...
/* 在PPC原生DUART上面发现一个bug
 * 如果在串口初始化完成之前有一个字符已经输入，并触发了timeout中断
 * 初始化过程在进行了rx fifo reset后，fifo为空，但是timeout事件没有被清除
 * 这时初始化串口后，一旦开始工作后，而且没有rx数据，timeout中断事件会持续上报。
 * ns16550vxbIntRd2判断LSR[DR]为0,不会进行fifo读取。
 * 这时会进入一个死锁状态，中断触发后，引发ns16550vxbIntRd2空调用，再进中断，再进ns16550vxbIntRd2
 * cpu会被完成占用
 * 为了解决这个问题，在进行rx fifo reset后，强制把16550的fifo读空
 */
static void zmd_16550_fix(NS16550VXB_CHAN * pChan)
{
    int i;
    volatile UINT8 val;
	VXB_NS16550_SPIN_LOCK_ISR_TAKE(pChan);
    for(i=0; i<pChan->fifoSize; i++){
        REG_GET(RBR, pChan, val);
    }
...
LOCAL void ns16550vxbInt
	...
	VXB_NS16550_ISR_SET(pChan);
        ...
        *REG_SET(IER, pChan, pChan->ier)打开了相应通道的中断，造成异常触发中断，而这时
        *pChan->queueId没有初始化，系统挂死在isrDeferJobAdd.
		*/
