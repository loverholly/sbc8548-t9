...
#define INCLUDE_F2X_SYS_FLASH_ROUTINES
/* defines */
...
{
    F2X_GDESC * gd = (F2X_GDESC *)gid;
    F2X_RDESC * rd;
    int i;
    printf("handle[0x%x] size[0x%x]\r\n",gid,f2xGroupSize(gid));
    for(i=0; i<gd->ridArrayCount; i++){
        rd = gd->ridArray[i];
        printf("\tregion%d: base:%x size:%x blockSize:%x buswidth:%x type:%s\n", 
               i, (UINT32)rd->baseAdrs, 
               rd->regionSize, rd->chipBlockSize, rd->chipWidth, 
               (rd->type & F2X_WB32_TYPE)?"write burst":"normal");
    }
}
 ...
 * 通过获取devid判断芯片是否工作正常
 */
LOCAL STATUS f2xRegionBit(F2X_RID id, UINT16 devID)
{
    STATUS      status = OK;
    F2X_RDESC * pRDesc = (F2X_RDESC *)id;
    char cmd[8] = {0, 0x90, 0, 0x90, 0, 0x90, 0, 0x90};
    char devid[8];
    UINT32 ofs, i;
    /* if(pRDesc->regionWidth  */
    if(2 != pRDesc->chipWidth){
        printf("not support chipwidth %d", pRDesc->chipWidth);
        return ERROR;
    }
#ifndef NO_INTERNAL_SEM
    semTake (pRDesc->semId, WAIT_FOREVER);
#endif
    
    f2xRegionWidthWrite (id, pRDesc->cmdUnlock1, pRDesc->offUnlock1, FALSE);
    f2xRegionWidthWrite (id, pRDesc->cmdUnlock2, pRDesc->offUnlock2, FALSE);
    f2xRegionWidthWrite (id, cmd, pRDesc->offUnlock1, FALSE);
    ofs = pRDesc->regionWidth;
    f2xRegionWidthRead(id, devid, pRDesc->regionWidth, FALSE);
    ofs = ofs >> 1;
    for(i=0; ofs>0; i++){
        UINT16 * p = (UINT16 *)devid;
        if(p[i] != devID){
            status = ERROR;
        }
        ofs = ofs >> 1;
    }
    f2xRegionWidthWrite (id, id->cmdReset, 0, FALSE);
#ifndef NO_INTERNAL_SEM
    semGive (pRDesc->semId);
#endif     
    return (status);
...
/**
 * 强制将各个group置于reset状态
 * flash的program buffer状态如果写入write buffer数据还没写完，软复位或被打断后，flash处于一个错误状态无法恢复。
 * 可以通过terminal write buffer命令强制关闭
 */
void f2xGroupReset(F2X_GID gid)
{
    ...
    F2X_RDESC * rd;
    char        statusArray[F2X_MAX_REGION_WIDTH];
    char        statusArray1[F2X_MAX_REGION_WIDTH];
    int         numChips;
    UINT32         tickNow;
    int i;
    int         chipIndex;
    char *      pChip = statusArray;
    char *      pData = statusArray1;
    volatile char        statusByte;
    char        dataByte = 0;
    for(i=0; i<gd->ridArrayCount; i++){
        memset(statusArray, 0, sizeof(statusArray));
        memset(statusArray1, 0, sizeof(statusArray));
        tickNow = tickGet();
      reset_again:
        rd = gd->ridArray[i];
        numChips = rd->regionWidth / rd->chipWidth;
        f2xRegionWidthRead (rd, statusArray, 0, FALSE); /* 读取状态1 */
        f2xRegionWidthRead (rd, statusArray1, 0, FALSE); /* 读取状态2 */
        if(0 == memcmp(statusArray1, statusArray, sizeof(statusArray1))){
                /* 不在status状态，不需要复位 */
                /* 某些时间write buffer的buffer还没有填满，status不会显示任何错误，但做任何其它操作会将该错误显现出来 */
            f2xRegionBit(rd, 0);
                /* 不管什么状态，给个write buffer reset 最保险 */
            f2xRegionWidthWrite (rd, rd->cmdUnlock1, rd->offUnlock1, FALSE);
            f2xRegionWidthWrite (rd, rd->cmdUnlock2, rd->offUnlock2, FALSE);
            f2xRegionWidthWrite (rd, rd->cmdReset, rd->offUnlock1, FALSE);
            continue;
        }else{
                /* 检查toggle，如果在工作模式，等待之 */
            for (chipIndex = 0; chipIndex < numChips; chipIndex++)
            {
                switch (rd->chipWidth)
                {
                    case 1:
                        dataByte = (char)*(UINT8 *)pData;
                        statusByte = (char)*(UINT8 *)pChip;
                        break;
                    case 2:
                        dataByte = (char)*(UINT16 *)pData;
                        statusByte = (char)*(UINT16 *)pChip;
                        break;
                    case 4:
                        dataByte = (char)*(UINT32 *)pData;
                        statusByte = (char)*(UINT32 *)pChip;
                        break;
                    default:
                        break;
                ...
                if((dataByte & 0x2)){ /* write buffer abort模式 */
                        /* 虽然是在RDY状态，但是还是有可能在write buffer过程当中，所以强行做一个write to buffer abort reset */
                    f2xRegionWidthWrite (rd, rd->cmdUnlock1, rd->offUnlock1, FALSE);
                    f2xRegionWidthWrite (rd, rd->cmdUnlock2, rd->offUnlock2, FALSE);
                    f2xRegionWidthWrite (rd, rd->cmdReset, rd->offUnlock1, FALSE);
                }else{
                        /* 有erase/program操作正在进行，等一阵子 */
                    if(tickGet() - tickNow <  (UINT32)(sysClkRateGet() * F2X_TIMEOUT)){
                            /* 还没有超时，重新读状态 */
                        goto reset_again;    
                    }
                }
...
/** 
 * 如果flash正常在erase/program操作，等其操作结束，如果不在强制将所有tmd置为reset状态
 * 
 */
#define F2X_MTD_MAX_NUMBER 4
#define MAX_NORFLASH_TFFS_PARTITION 4
extern F2X_GID f2xMtd[F2X_MTD_MAX_NUMBER];

extern F2X_GID f2xTffsGid[MAX_NORFLASH_TFFS_PARTITION];
...
void mtdReset()
{
    ...
    for(i=0; i<F2X_MTD_MAX_NUMBER; i++){
        if((f2xMtd[i])){
            f2xGroupReset(f2xMtd[i]);
        }
    }
    for(i=0; i<MAX_NORFLASH_TFFS_PARTITION; i++){
        if((f2xTffsGid[i])){
            f2xGroupReset(f2xTffsGid[i]);
        }
    }
}
